...
class GameState():
    def __init__(self):
        ...
        self.pins = []
        self.checks = []
        self.currentCastlingRight = castleRight(True, False, True, True)
    ...
    def check_only_kings(self, board):
        # Список допустимых фигур
        allowed_figures = {"bK", "wK", "--"}

        for row in board:
            for piece in row:
                if piece not in allowed_figures:
                    return False
        return True

    def check_only_one_black_knight(self, board):
        # Список допустимых фигур
        allowed_figures = {"bK", "bN", "wK", "--"}
        knight_count = 0
    ...
    def getValidMoves(self):
        ...
        if len(moves) == 0 or self.check_only_kings(self.board) or self.check_only_one_black_knight(self.board):
            if self.inCheck:
                self.checkMate = True
            else:
                self.staleMate = True
